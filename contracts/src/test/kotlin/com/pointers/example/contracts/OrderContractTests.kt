...
class OrderContractTests: ContractTestsUtil() {
	...
	private var conanDatabase: CordaPersistence? = null
	private var conanLedgerServices: ServiceHub? = null
	
	private var junkoDatabase: CordaPersistence? = null
	private var junkoLedgerServices: ServiceHub? = null
	val notaryServices = MockServices(
		...
		fun makeDatabaseAndLedgerServices(){
			val (a, b) = MockServices.Companion.makeTestDatabaseAndMockServices(
			listOf("com.pointers.example.contracts"),
			...
			conanDatabase = a
			conanLedgerServices = b
			val (c, d) = MockServices.Companion.makeTestDatabaseAndMockServices(
					listOf("com.pointers.example.contracts"),
	...
	fun `create order`() {
	    ledgerServices.ledger {
		    ...
		    val productStaticPointer = StaticPointer(productStateAndRef.ref, devilFruit.javaClass)
	...
	fun `create order and resolve product pointer in canon vault`() {
		...
		val createProductTx = conanDatabase!!.transaction {
			...
			builder.addOutputState(devilFruit)
	...
	fun  retrieveProductFromCanonVault(productPointer: StaticPointer<Product>): SignedTransaction?{
	...
	fun `create order and resolve product pointer in junko vault`() {
		...
		val productTransaction: SignedTransaction? = retrieveProductFromCanonVault(orderStateInJunkoVault.productPointer)
	...
	fun `create order, update product and verify the price order in conan vault`() {
		...
		val updateProductTx = conanDatabase!!.transaction {
			...
			builder.addOutputState(devilFruitUpdated)
	...
	fun `create order, update product and verify the price order in junko vault`() {
		...
		val productTransaction: SignedTransaction? = retrieveProductFromCanonVault(orderStateInJunko.productPointer)
		junkoLedgerServices!!.recordTransactions(listOf(productTransaction!!))
